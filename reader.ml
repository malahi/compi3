 ...
 let rec sexp_p s =   
     let nested = PC.caten (PC.caten sexp_comment_p (make_spaced (PC.disj_list [_dot_sexp_tag_ ;  _sexp_tag_ ; dot_list_p ; bool_p ; char_p ; number_p ; symbol_p ; string_p ; nil_p ; list_p ; q_p ; dot_list_p ; square_dot_list_p ; square_list_p ; vector_p])) ) sexp_comment_p in
       PC.pack nested (fun ((s , e) , k) -> e) s
  
 and list_p  s      = (PC.pack (PC.caten (fun (s) -> list_h_p false false s ) (PC.maybe (PC.word "..."))) (fun(l,r)->l)) s
 and square_list_p s= (PC.pack (PC.caten (fun (s) -> list_h_p true false s ) (PC.maybe (PC.word "..."))) (fun(l,r)->l)) s 
 
 and dot_list_p       s = (PC.pack (PC.caten (fun (s) -> dot_list_h_p false false s ) (PC.maybe (PC.word "..."))) (fun(l,r)->l)) s               
 and square_dot_list_p s= (PC.pack (PC.caten (fun (s) -> dot_list_h_p true false s ) (PC.maybe (PC.word "..."))) (fun(l,r)->l)) s                  
 ...
 (* ---------------------========================== three dots ==============================------------------------- *)
 and sexp_tag_p s =   
     let nested = PC.caten (PC.caten sexp_comment_p (make_spaced (PC.disj_list [dot_list_tag_p ; square_dot_list_tag_p  ; bool_p ; char_p ; number_p ; symbol_p ; string_p ; nil_p ; list_tag_p ; q_p ;  square_list_tag_p ]) )) sexp_comment_p in
             PC.pack nested (fun ((s , (e )) , k) -> e) s
 ...
 let read_sexpr string = 
   let l = string_to_list string in
       let (e , s) = (make_spaced sexp_p) l in
         match s with
         | [] -> e
 ...
 let read_sexprs string = 
   let (e , s) = (make_spaced sexp_comment_p) (string_to_list string) in
     ...
     | _ -> let l = string_to_list string in 
             match l with 
             | [] ->  [] 
             | _  ->  let (e , s) = (make_spaced (PC.plus sexp_p)) l in
                     match s with
                     | [] -> e
                     | _ -> raise PC.X_no_match;;
  (* end;; *)
